...
constrainFunc findFunc(char *path, char *funcName, size_t length)
{
   ...
   if (libhandle == NULL) {
      ERROR("Cannot find constrain module: %s\n", path);
   } else if (
      (version = (long int *)findName(libhandle, idstring, sizeof(idstring) - 1)
      ) == NULL
   ) {
      ERROR("Cannot establish version of constrain module\n");
      closeLib();
   } else if (MAJORVER(*version) != MAJOR) {
      ERROR("Constrain module is not designed for this program\n");
      closeLib();
   } else if (MINORVER(*version) < MINOR) {
      ERROR("Constrain module is designed for an older version of this program\n");
      closeLib();
   } else if (
      (func = (constrainFunc) findName(libhandle, funcName, length)
      ) == NULL
   ) {
      ERROR("Constrain module does not define constraints\n");
      func = noconstraints;
      closeLib();
   }
   if (func == noconstraints)
      ERROR("Defaulting to no constraints\n");
#endif
...
void noconstraints(int dels, double a[], int nt, int nm, int nr, int nb)
{
}
         ...
         switch (child) {
            case 0:
               ...
               if (execvp(makeBinary, makeargv) == -1) {
                  ERROR("Cannot load strain compiler %s: %s\n", makeBinary,
                     strerror(errno));
                  exit(errno);
               }
               break; /* gratuitous */
         ...
         switch (child) {
            case 0:
               ...
               if (
                  ...
                  ERROR("Failed to launch editor: %s: %s\n", editor,
                     strerror(errno));
                  exit(errno);
               }
               break; /* gratuitous */
...
STATIC int prepScriptfile(char *scriptfile)
{
   ...
   " * as you would in the program.  For example: TQC3 = TQC4    *\n"
   " * You may also use parentheses: TQC(3) = TQC(j), which sug- *\n"
