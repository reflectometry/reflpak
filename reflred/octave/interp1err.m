...
## usage: yi = interp1err(x, y, xi [, 'method' [, 'extrap']])
##
...
##    fixed extrapolation
## 2002-01-24 Paul Kienzle
##    extended to handle error dy for linear case

function [yi,yierr] = interp1err(x, y, yerr, xi, method, extrap)
  ...
  if strcmp(extrap,"extrap")
    range=1:size(xi,1);
    yierr = yi = zeros(size(xi,1), size(y,2));
  else
    range = find(xi >= minx & xi <= maxx);
    yierr = yi = extrap*ones(size(xi,1), size(y,2));
    if isempty(range), 
  ...
  elseif strcmp(method, 'linear')
    ...
    yierr(range,:) = sqrt( s.^2.*yerr(idx,:).^2 + (1-s).^2.*yerr(idx+1).^2 );
