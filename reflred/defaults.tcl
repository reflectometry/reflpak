...
if { "$RESOURCE_VERSION" != "$VERSION" } {
    message -warn "~/.mlayer (v$VERSION) does not match mlayer.tcl (v$RESOURCE_VERSION) and so it will be ignored"
    return 0
}
...
# what markers to use for the data lines
if { $::MAGNETIC } {
    set marker(da) circle
    set marker(dd) square
    set marker(db) triangle
    set marker(dc) diamond
    set marker_size(da) 2
    set marker_size(dd) 2
    set marker_size(db) 4
    set marker_size(dc) 4
} else {
    set marker(d)  circle
    set marker_size(d) 2
}
