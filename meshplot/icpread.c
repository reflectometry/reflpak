...
static const char *scan_token(const char header[], int n, char token[])
{
  ...
  *val = '\0';
  ...
  if (*header == '\'') {
    ...
    if (*header) {
      /* Missing closing quote, so signal an error */
      *token = '\0';
      return NULL;
    }
...
const char *icp_error(int code)
{
  ...
  if (code > _ICP_LAST_CODE_ || code < _ICP_FIRST_CODE_) {
    ...
    return msg[code-_ICP_FIRST_CODE_+1];
  }
}
		   ...
		   int *linenum)
{
  ...
  *linenum = 1;
  ...
  /* Copy lines until one after the "Mot:" line */
  seen_motor_line = 0;
  offset = 0;
  while (!gzeof(infile)) {
...
int icp_framesize(FILE *infile, int *rows, int *columns, int *values)
{
  ...
  /* last row does not end in semicolon; last column does not end in comma */
  *rows = nsemicolon + 1;
...
int icp_readdetector(FILE *infile, int rows, int columns, Counts frame[], int *linenum)
{
  ...
  /* First character in a line in the table is a space; if there are two
     blanks at the start of the line then for some reason ICP dropped 
     the frame.
   */
  ...
  if (ch == ' ') return ICP_GOOD; /* Empty frame */
...
int main(int argc, char *argv[])
{
  ...
  for(;;) {
    ...
    // for (i = 0; i < nc; i++) printf("%d ",frame[i]); printf("\n");
    if (status < 0) break;
    framenumber++;
  }
  icp_close(f);
  ...
  if (status == ICP_EOF) {
    ...
    printf("Final frame:\n");
    for (i = 0; i < nc; i++) printf("%d ",frame[i]);
    printf("\n");
    return 0;
  } else {
    printf("Error line %d: %s\n",line,icp_error(status));
